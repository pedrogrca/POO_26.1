...
Abra o menu com `python main.py` e escolha a opção **3 — Carregar dados para
demonstração**. Em seguida, entre com um dos perfis abaixo:

| Perfil | Login | Senha | Demonstra na aula |
|---|---|---|---|
| Organizador | `admin` | `admin` | CRUD de categoria, local, evento, lote e cupom; consultas de inscrições e check-in. |
| Participante | `maria` | `123` | Pesquisa de eventos, visualização de lotes, compra com cupom e cancelamento. |

O botão cria uma base inicial apenas se o usuário `admin` ainda não existir.
Todos os dados criados pela interface ficam em `data/*.json`, inclusive após
